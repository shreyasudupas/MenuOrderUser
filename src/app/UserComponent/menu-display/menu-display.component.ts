...
import { MenuCartData, MenuColumnDetail, MenuDisplayReponse } from 'src/app/Models/menu-display/menu-display-response';
...
export class MenuDisplayComponent extends BaseComponent<MenuDisplayReponse>{
    ...
    menuColumns:Array<MenuColumnDetail>;
    menus:Array<any>;
    ...
    vendorName:string = '';
    userLocality:string = '';
    toastMessagePosition:string = "top-right";
    ...
    ngOnInit(){
        ...
        if(this.vendorId == undefined || this.vendorId == null){
          ...
          this.router.navigateByUrl('/user/vendorlist');
            ...
            //Get Locality of user selected
            this._broadcastService.getUserLocality().subscribe(res=>{
              this.userLocality = res;
            });
            ...
            //Setting Vendor Menu Details
            let RequestResource1 = new RequestResource();
            //RequestResource1.requestUrl = env.menuAPI+'GetMenuList?VendorId='+this.vendorId;
            RequestResource1.requestUrl = env.vendorAPI + '/menudetails?locality=' + this.userLocality + '&vendorId=' + this.vendorId;
            ...
            RequestResource2.requestUrl = env.BasketAPI + '?username=' + this.authService.getUserInformation().profile?.Username;
            ...
            this.getForkItems(forkRequest).subscribe(([menuResult,basketCacheResult])=>{
              ...
              let cacheMenuBasket = basketCacheResult;
                // if(results.length>0){
                // }
                //get menu list items
                if(menuResult != null){

                    this.menuColumns = this.menuResponse.menuColumnDetail;
    ...
    addItemsoCart(itemSelected:any){
        ...
        if(this.menuCartItems[index].quantity<20){
          ...
          if(menu != undefined && menu != null){
            ...
            this.action = null;
            this.Create(body).subscribe((response:any)=>{
              if(response == true){
    ...
    removeItemsFromCart(itemeSelected:any){
      ...
      if(this.menuCartItems[index].quantity>0){
        ...
        if(menu != undefined){
            ...
            this.versionUrl = env.BasketAPI;
            this.action = null;
            this.UpdateItem(body).subscribe((response:any)=> {
                if(response == true){
